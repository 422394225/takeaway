...
public class EmailUtils {
	...
	private static final String MAIL_STROE = prop.get("mail.store");
	...
	private static final Message initMessage() {
		...
		try {
			message.setFrom(new InternetAddress(
					MimeUtility.encodeText(MAIL_DISPLAY_SENDNAME) + '<' + MAIL_DISPLAY_SENDMAIL + '>'));
	...
	private static void saveEmail() {
		try {
			File file = new File(MAIL_STROE);
			if (!file.exists()) {
				file.mkdirs();
			}
			//  将该邮件保存到本地
			OutputStream out = new FileOutputStream(MAIL_STROE + "email" + System.currentTimeMillis() + ".eml");
			message.writeTo(out);
			out.flush();
			out.close();
		} catch (Exception e) {
