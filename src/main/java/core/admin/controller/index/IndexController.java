...
public class IndexController extends Controller {
	...
	public void index() {
		...
		//根据数据库每条信息的创建时间查询前一周的数据
		list = Data.dao.find("SELECT * FROM t_data WHERE CREATE_TIME> DATE_ADD(NOW(),INTERVAL -6 DAY)");
		//初始化数据
		...
		//处理数据
		for(Data d:list){
			String week =""+d.<Integer>get("WEEK");
			String hour = ""+d.<Integer>get("REF_HOUR");
			rs.put(week+hour,"["+week+","+hour+","+d.<Integer>get("CALLBACK_COUNT")+"]");
